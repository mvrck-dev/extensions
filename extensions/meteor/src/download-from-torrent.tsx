...
import { addTorrent } from "./utils/aria2rpc";
...
export default function Command() {
  ...
  const [initialTorrentPath, setInitialTorrentPath] = useState<string | null>(
    null,
  );
  ...
  // Initialize daemon and detect selected torrent file on mount
  useEffect(() => {
    async function init() {
      try {
        await startAria2Daemon();

        // Detect selected torrent file in Finder
        try {
          const selectedItems = await getSelectedFinderItems();
          const torrentFile = selectedItems.find((item) =>
            item.path.toLowerCase().endsWith(".torrent"),
          );
          if (torrentFile) {
            setInitialTorrentPath(torrentFile.path);
          }
        } catch (err) {
          // Ignore if Finder is not focused or has no selection
          console.log("Could not read Finder selection:", err);
        }
  ...
  const handleSubmit = async (values: FormValues) => {
    ...
    if (!values.torrent || values.torrent.length === 0) {
      await showToast({
        style: Toast.Style.Failure,
    ...
    const toast = await showToast({
      ...
      title: "Adding torrent download...",
    });

    try {
      const filePath = values.torrent[0];
      if (!fs.existsSync(filePath)) {
        throw new Error("Torrent file does not exist.");
      }

      // Read file and encode in base64
      const torrentBase64 = fs.readFileSync(filePath).toString("base64");
      ...
      // Call addTorrent RPC
      await addTorrent(torrentBase64, options);

      toast.style = Toast.Style.Success;
      toast.title = "Added successfully";
      toast.message = "Torrent download queued.";
      ...
      toast.title = "Failed to add torrent";
  ...
  return (
    ...
    >
      <Form.FilePicker
        ...
        defaultValue={initialTorrentPath ? [initialTorrentPath] : undefined}
        info="Select the .torrent file to download. Automatically prefilled if you selected a torrent file in Finder before launching."
      />
      <Form.FilePicker
        id="dirs"
        title="Save To"
      ...
      <Form.TextField
        id="rename"
        title="Rename File"
        placeholder="Optional name to rename the downloaded content..."
      />
      <Form.Dropdown
        id="split"
